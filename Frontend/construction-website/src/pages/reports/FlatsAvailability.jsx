...
import { reportsAPI, projectsAPI } from "../../utils/apiService"; 
  ...
  return (
    <span className="inline-flex flex-col gap-[1px] ml-1.5 relative top-[1px]">
      <svg width="7" height="5" viewBox="0 0 7 5" fill="none">
        <path d="M3.5 0L7 5H0L3.5 0Z" fill={active && dir === "asc" ? "#334155" : "#94a3b8"} />
      </svg>
      <svg width="7" height="5" viewBox="0 0 7 5" fill="none">
        <path d="M3.5 5L0 0H7L3.5 5Z" fill={active && dir === "desc" ? "#334155" : "#94a3b8"} />
      </svg>
    </span>
  );
}
...
const COLS = [
  ...
  { key: "project",   label: "PROJECT", render: (v) => v?.name ?? "—" },
  {
    ...
    render: (v) => {
      ...
      return (
        <span className={`px-3 py-0.5 rounded-full text-xs font-semibold ${STATUS_BADGE[key] ?? ""}`}>
          {STATUS_LABEL[key] ?? v}
        </span>
      );
    },
...
export default function FlatsAvailability() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      // API returns { data: { stats, allFlats } }
      const res = await reportsAPI.flatsAvailability(params);
      let flats = res?.data?.data?.allFlats ?? [];
      const rawStats = res?.data?.data?.stats ?? [];

      // Filter by status on frontend if selected
      if (status) {
        flats = flats.filter((f) => f.status?.toLowerCase() === status.toLowerCase());
      }
  ...
  const sorted = results
    ? sortKey
      ? [...results].sort((a, b) => {
          const av = sortKey === "project" ? (a.project?.name ?? "") : (a[sortKey] ?? "");
          const bv = sortKey === "project" ? (b.project?.name ?? "") : (b[sortKey] ?? "");
          return sortDir === "asc"
  ...
  // Stat counts from API stats array: [{ _id: 'available', total: 5 }, ...]
  const getCount = (s) => stats.find((x) => x._id === s)?.total ?? 0;

  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-[22px] font-bold text-slate-800">Flat Availability Report</h1>
      ...
      {results === null && !loading ? (
        <div className="bg-white rounded-xl border border-slate-200 py-20 text-center text-slate-400 text-sm">
          Select filters and click Generate to view flat availability
        </div>
      ) : loading ? (
        <div className="bg-white rounded-xl border border-slate-200 py-20 text-center text-slate-400 text-sm">
          Loading...
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl border border-slate-200 px-6 py-5">
              ...
              <p className="text-xl font-bold text-emerald-600 mt-1">{getCount("available")}</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 px-6 py-5">
              <p className="text-sm text-slate-500">Booked</p>
              <p className="text-xl font-bold text-blue-600 mt-1">{getCount("booked")}</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 px-6 py-5">
              <p className="text-sm text-slate-500">Sold</p>
              <p className="text-xl font-bold text-red-500 mt-1">{getCount("sold")}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  {slice.length === 0 ? (
                    ...
                    slice.map((row, i) => (
                      <tr key={i} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50 transition-colors">
                        {COLS.map((c) => (
                          <td key={c.key} className="px-6 py-[17px] text-sm text-slate-700 whitespace-nowrap">
            ...
            <div className="flex items-center justify-between px-6 py-3.5 border-t border-slate-100 flex-wrap gap-3">
              <p className="text-xs text-slate-500">
                ...
                {Math.min(safePage * PER, sorted.length)} of {sorted.length} results
              </p>
              {pages > 1 && (
