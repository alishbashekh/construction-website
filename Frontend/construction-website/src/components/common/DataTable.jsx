...
export default function DataTable({
  ...
  function updateSearch(val) { setSearch(val); setPage(1); }
  function updateFilter(key, val) { setFilterVals((p) => ({ ...p, [key]: val })); setPage(1); }
    ...
    () =>
      afterSearch.filter((row) =>
        filters.every((f) => {
          ...
          return !val || String(row[f.key] ?? "").toLowerCase() === val.toLowerCase();
        }),
      ),
  ...
  const pageSlice = afterSort.slice((safePage - 1) * rowsPerPage, safePage * rowsPerPage);
  ...
  return (
    <div className="flex flex-col gap-5 w-0 min-w-full">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        {onAddClick && (
          ...
          >
            {addIcon ?? <UserRoundPlus className="w-5 h-5" />}
            {addLabel}
          </button>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative ml-auto">
          ...
          />
          {search && (
            <button onClick={() => updateSearch("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden w-full min-w-0">
        {/* Horizontal scroll lives HERE, inside the card — not on the page */}
              ...
              <tr className="border-b border-slate-200 bg-slate-50/60">
                {columns.map((col) => (
                  <th
                    ...
                    className={`px-4 py-3.5 text-left text-[10px] font-bold text-slate-500 uppercase tracking-widest whitespace-nowrap select-none ${col.sortable ? "cursor-pointer hover:text-slate-700" : ""}`}
                  >
                    {col.label}
                ...
                pageSlice.map((row, i) => (
                  ...
                  >
                    {columns.map((col) => (
                      <td
                        key={col.key}
                        className="px-4 py-[13px] text-[12.5px] text-slate-700 whitespace-nowrap"
                      >
